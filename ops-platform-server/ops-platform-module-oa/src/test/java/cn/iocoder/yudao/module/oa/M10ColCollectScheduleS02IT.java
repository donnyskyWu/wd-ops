...
class M10ColCollectScheduleS02IT extends OaITBase {
    ...
    void cronScanRunsDueTask() throws Exception {
        ...
        org.junit.jupiter.api.Assertions.assertEquals("RUNNING", afterRun.getStatus());
    }

    @Test
    @DisplayName("M10-COL-S-02: PENDING 任务不被 cron 扫描")
    void cronScanSkipsPendingTask() throws Exception {
        MvcResult createResult = mockMvc.perform(post("/admin-api/oa/collect/task/create")
                        .header("Authorization", AUTH)
                        .header("X-Tenant-Id", TENANT)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @DisplayName("M10-COL-S-02: STOPPED 任务不被 cron 扫描")
    void cronScanSkipsStoppedTask() throws Exception {
        MvcResult createResult = mockMvc.perform(post("/admin-api/oa/collect/task/create")
                        .header("Authorization", AUTH)
                        .header("X-Tenant-Id", TENANT)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(post("/admin-api/oa/collect/task/" + taskId + "/stop")
                        .header("Authorization", AUTH)
                        .header("X-Tenant-Id", TENANT))
                .andExpect(jsonPath("$.code").value(0));
