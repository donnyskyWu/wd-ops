...
public class DashboardServiceImpl implements DashboardService {
    ...
    @Override
    @Transactional
    @AuditLog(module = "M6-dashboard", action = "full-update")
    public void updateFull(DashboardUpdateReq req) {
        DashboardDO entity = getRequired(req.getId());
        if (req.getDashboardName() != null) {
            entity.setDashboardName(req.getDashboardName());
        }
        if (req.getDashboardType() != null) {
            entity.setDashboardType(req.getDashboardType());
        }
