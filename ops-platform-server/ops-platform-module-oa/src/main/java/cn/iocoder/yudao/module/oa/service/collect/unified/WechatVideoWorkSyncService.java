...
public class WechatVideoWorkSyncService {
    ...
    @Transactional
    public int syncWorks(Long oaAccountId) {
        ...
        for (int page = 0; page < 100; page++) {
            Map<String, Object> payload = unifiedCollectorApiClient.getWechatVideoList(
            ...
            for (JSONObject video : videos) {
                if (upsertWork(tenantId, oaAccountId, video, now)) {
                    synced++;
                }
            }
            if (!hasMore(payload)) {
    ...
    private boolean upsertWork(Long tenantId, Long accountId, JSONObject video, LocalDateTime now) {
        String videoId = firstNonBlank(video, "export_id", "video_id", "object_id", "id");
        if (StrUtil.isBlank(videoId)) {
            return false;
        }
        WechatVideoWorkDO existing = wechatVideoWorkMapper.selectOne(
                new LambdaQueryWrapper<WechatVideoWorkDO>()
                        ...
                        .eq(WechatVideoWorkDO::getVideoId, videoId));
        if (existing == null) {
            WechatVideoWorkDO entity = new WechatVideoWorkDO();
            entity.setAccountId(accountId);
            entity.setVideoId(videoId);
            applyWorkFields(entity, video, now);
            ConfigTenantSupport.fillCreate(entity);
            wechatVideoWorkMapper.insert(entity);
            return true;
        }
        applyWorkFields(existing, video, now);
        ConfigTenantSupport.fillUpdate(existing);
        wechatVideoWorkMapper.updateById(existing);
        return true;
    }

    private void applyWorkFields(WechatVideoWorkDO entity, JSONObject video, LocalDateTime now) {
        ...
        entity.setPlayCount(firstInt(video, "read_count", "play_count", "playCount"));
        entity.setLikeCount(firstInt(video, "like_count", "likeCount"));
        entity.setShareCount(firstInt(video, "forward_count", "share_count", "shareCount"));
        entity.setCommentCount(firstInt(video, "comment_count", "commentCount"));
        entity.setCollectCount(firstInt(video, "fav_count", "favorite_count", "collectCount"));
    ...
    private List<JSONObject> extractVideos(Map<String, Object> payload) {
        ...
        Object raw = firstPresent(payload, "videos", "items", "list");
