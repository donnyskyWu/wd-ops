...
- [x] ~~已绑定实名人被新账号选择时弹"强制替换"确认框~~ → **废止（ADR-062）**：实名人同时 1:N，可直接多绑
- [ ] 已绑定手机/手机卡被新账号选择时弹"强制替换"确认框（仍有效）
- [ ] 强制替换需 `reason` 字段（错误码 2014；仅手机/卡）
...
- [x] `<RealNameSelect />` **不**默认过滤已绑定（ADR-062 · 同时 1:N）
- [ ] `<PhoneSelect />` 默认过滤"已绑定"实体
- [ ] `<SimCardSelect />` 同上
- [ ] `<CompanySelect />` 同上
- [ ] `<AccountSelect />` 同平台过滤
...
- [x] ~~选择"已绑定"实名人 → 强制替换~~ → **废止（ADR-062）**：同实名人再绑新账号应直接成功
- [ ] 选择"已绑定"手机/手机卡 → 弹出"强制替换"确认框
- [ ] 确认 → 提示填 `reason`（最小 5 字）
- [ ] 提交 → 旧账号解绑 + 新账号绑定 + 审计日志
- [ ] 未填 reason → 错误码 1500
- [ ] `forceReplace=false` 选已绑定手机/卡 → 错误码 1502
- [x] 同实名人绑账号 A 后再建 B（无 forceReplace）→ 成功（ADR-062）
