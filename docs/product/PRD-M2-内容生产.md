...
| 关联 API | `docs/engineering/API-M2-内容生产.md` |
...
管理**内容从创作到发布**的全生命周期：SOP 模板编排（DAG + 并行）→ 任务执行 → 三级审核 → 自动发布 → 知识沉淀。
...
SOP 任务实例，跟踪任务执行状态、节点进度、审核结果、SLA 超时告警。

#### 4.2.2 主流程

1. 业务计划关联 SOP 模板 → 系统自动创建任务实例
2. 按 DAG 顺序激活节点
