...
import { debounceSearch, useMockFallback } from './selector-utils'
...
const loadList = async (keyword: string) => {
  ...
  try {
    ...
    options.value = useMockFallback(list, mockCompanyList)
  } catch (e) {
    console.warn('[CompanySelect] fallback to mock:', e)
    options.value = import.meta.env.VITE_USE_MOCK === 'true' ? mockCompanyList : []
  } finally {
    loading.value = false
  }
}
