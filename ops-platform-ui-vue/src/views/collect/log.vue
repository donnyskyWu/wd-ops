...
<template>
  <div class="collect-log-page">
    ...
    <el-drawer
      ...
      size="80%"
      destroy-on-close
      class="collect-log-detail-drawer"
    >
      <CollectLogDetailPanel v-if="viewingLogId" :log-id="viewingLogId" />
    </el-drawer>
  </div>
</template>
...
import CollectLogDetailPanel from './log-detail.vue'
import {
  getCollectLogPage,
  getCollectTaskPage,
} from '@/api/collect'
...
const detailDrawerTitle = ref('采集日志详情')
...
const handleViewDetail = (row: any) => {
  detailDrawerTitle.value = `采集日志详情 - ${row.taskName || row.id}`
...
<style lang="scss">
.collect-log-detail-drawer.el-drawer {
  min-width: 720px;
}
</style>
