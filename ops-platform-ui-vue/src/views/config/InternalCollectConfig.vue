<template>
  <div class="internal-collect-config">
    <ContentWrap title="内部采集配置" subtitle="各平台账号&企微API&个微-奥创API配置">
      <el-alert 
        title="内部采集配置用于管理各平台的数据采集API连接信息" 
        type="info" 
        :closable="false" 
      ...
      <el-tabs v-model="activeTab" @tab-change="handleTabChange">
        <el-tab-pane label="平台账号配置" name="platform" />
        <el-tab-pane label="企微API配置" name="wework" />
        <el-tab-pane label="个微-奥创API" name="wechat" />
      </el-tabs>
      ...
      >
        ...
        <el-table-column prop="syncFrequency" label="采集频率" width="100" align="center">
          <template #default="{ row }">
    ...
    >
      ...
      >
        ...
        <el-form-item label="平台类型" prop="platformType">
          <DictSelect v-model="formData.platformType" dict-type="dict_platform_type" placeholder="请选择平台类型" style="width: 100%" />
        </el-form-item>
        ...
        <el-form-item label="采集频率" prop="syncFrequency">
          <DictSelect v-model="formData.syncFrequency" dict-type="dict_sync_frequency" placeholder="请选择采集频率" style="width: 100%" />
        </el-form-item>
...
const displayList = computed(() =>
  configList.value.filter((item) => !item.subType || item.subType === activeTab.value)
)
...
const handleToggleStatus = async (row: ConfigItem) => {
  ...
  try {
    ...
    await internalCollectApi.update({ id: row.id, status: newStatus })
