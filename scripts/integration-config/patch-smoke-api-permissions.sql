...
-- Root cause: system_menu rows exist (6137/6138/6139/6149) but system_role_menu missing for role_id=1
-- after partial seed or TRUNCATE — author (6155) may remain while account/log/dict 403.
-- API smoke probes: wechat_account_list, login_log, dict_list (POST-MDB §23 #2).
...
VALUES
  ...
  (6138, '登录日志', 'oa:log:login', 2, 2, 6105, 'system-log/login', 'ep:document', 'ops/system/LoginLog', 'SystemLogLogin', 0, b'1', b'1', b'1', 'integration', 2),
  (6139, '操作日志', 'oa:log:operation', 2, 3, 6105, 'system-log/operation', 'ep:document', 'ops/system/LogManage', 'SystemLogOperation', 0, b'1', b'1', b'1', 'integration', 2),
...
INSERT IGNORE INTO system_role_menu (id, role_id, menu_id, creator, tenant_id) VALUES
  ...
  (70028, 1, 6138, 'integration', 1),
  (70029, 1, 6139, 'integration', 1),
...
INNER JOIN system_menu m ON m.id IN (6137, 6138, 6139, 6149, 6174) AND m.deleted = 0
