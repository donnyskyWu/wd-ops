#!/usr/bin/env python3
"""Generate UAT-FOOTBALL-E2E-20260704.md from probe JSON."""
...
JSON_PATH = ROOT / "docs/delivery/uat-football-e2e-20260704-probe.json"
MD_PATH = ROOT / "docs/delivery/UAT-FOOTBALL-E2E-20260704.md"
...
def main() -> int:
    ...
    for p in data.get("pages", []):
        ...
        lines.append(
            f"| {p.get('group', '')} | `#{p.get('hash', '')}` | {p.get('title', '')} | {status} | {api} | {notes} |"
        )
    lines += [
        "",
        "## Re-run",
